...
def _selected_combo_ids(all_combos: dict) -> list[str]:
    """
    Respect SELECTED_COMBOS from config. If not set, operate on all combos in sidecar.
    """
...
def main() -> None:
    ...
    if not all_combos:
        ...
        # still write passthrough artefact for downstream convenience
    ...
    for name, spec in config.BLEND_SPECS.items():
        ...
        if op == "capped_minmax":
            ...
            for cid in cids:
                ...
                # 2) Flags from blended thresholds (NOTE: no cap_delta arg here)
                ...
                sidecar["blends"][name][cid] = {
                    ...
                    # keep references to base adaptive flags (useful for further ops)
            ...
            for cid in cids:
                meta = all_combos[cid]
                # Prefer latest blended flags (from capped_minmax) if present; else adaptive
                prior = sidecar["blends"].get("CAPPED_minmax", {}).get(cid, {})
                if_flag = prior.get("if_blend_flag") or meta.get("if_flag")
                ae_flag = prior.get("ae_blend_flag") or meta.get("ae_flag")
                ...
                sidecar["blends"][name][cid] = {
                    ...
                    "ae_flag_dwell": dwell_cols["ae_dwell_col"], # AE with dwell
