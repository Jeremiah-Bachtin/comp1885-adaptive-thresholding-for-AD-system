...
"""
Generate adaptive thresholds, flags, and hybrid labels for IF×AE combos.

Inputs
------
- config/.env       (WINDOW_* lists, PERCENTILES_*, COMBOS_SCOPE, etc.)
- data CSV from config.DATA_PATH (must include: timestamp, if_score, lstm_score)

Outputs (to {RESULTS_RUN_DIR}/)
-------
- scores_with_thresholds.csv
    Contains:
      * Rolling adaptive thresholds for IF and AE (per (w,q))
      * Adaptive flags for IF (low-tail) and AE (high-tail)
      * Hybrid labels per combo (Point/Pattern/Compound)
      * Static flags (ensured if missing)
- columns_map.json
    Sidecar mapping each combo_id to the exact column names created.

Notes
-----
- Idempotent: re-running overwrites columns in the same run folder.
- Scope is driven by COMBOS_SCOPE (all | selected) in .env.
"""
...
from src.thresholding.rolling import compute_and_flag, make_colnames
...
def main() -> None:
    ...
    # ---- Build combos according to global scope ----------------------------
    ...
    # Factorise: unique IF and AE (compute rolling once per (w,q))
    ...
    # ---- Rolling warm-up setting -------------------------------------------
    minp = None if config.ADAPTIVE_MIN_PERIODS == "window" else 1

    # ---- Compute IF adaptive thresholds/flags ------------------------------
    for w_if, q_if in unique_if:
        tcol, fcol = make_colnames("if", w_if, q_if)
        compute_and_flag(df, "if_score", w_if, q_if, "if", direction="low", min_periods=minp)
        sidecar["if_settings"].append(
            {"window": w_if, "quantile": q_if, "thresh_col": tcol, "flag_col": fcol}
        )
    _log(f"Computed IF thresholds & flags for {len(unique_if)} (w,q) pairs.")

    # ---- Compute AE adaptive thresholds/flags ------------------------------
    for w_ae, q_ae in unique_ae:
        tcol, fcol = make_colnames("lstm", w_ae, q_ae)
        compute_and_flag(df, "lstm_score", w_ae, q_ae, "lstm", direction="high", min_periods=minp)
        sidecar["ae_settings"].append(
            {"window": w_ae, "quantile": q_ae, "thresh_col": tcol, "flag_col": fcol}
        )
    _log(f"Computed AE thresholds & flags for {len(unique_ae)} (w,q) pairs.")

    # ---- Hybrid labels per combo ------------------------------------------
    new_hybrids = 0
    for (w_if, q_if, w_ae, q_ae) in combos:
        _, if_flag = make_colnames("if", w_if, q_if)
        _, ae_flag = make_colnames("lstm", w_ae, q_ae)
        cid = combo_id(w_if, q_if, w_ae, q_ae)
        lbl = f"hybrid_label_{cid}"

        # Centralised hybrid construction
        df[lbl] = hybrid_from_flags(
            df[if_flag].astype("boolean"),
            df[ae_flag].astype("boolean"),
        )
        new_hybrids += 1

        t_if, _ = make_colnames("if", w_if, q_if)
        t_ae, _ = make_colnames("lstm", w_ae, q_ae)
        sidecar["combos"][cid] = {
            "if_window": w_if,
            "if_quantile": q_if,
            "ae_window": w_ae,
